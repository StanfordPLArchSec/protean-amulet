...
export DOCKER_DIR=$RVZR_DIR/docker/docker_protean; # Contains yamls!
...
export ST_ARCHSEQ_YAML=$DOCKER_DIR/docker_gem5_v1_final_cache_and_tlb.yaml;
# export ST_ARCHSEQ_YAML=$DOCKER_DIR/docker_gem5_v1_final_cache.yaml;
export ST_ARCHSEQ_SIMPLE_YAML=$DOCKER_DIR/docker_gem5_v1_final_cache_tlb_simple.yaml;
...
for i in $(seq 1 100); do
    # --verbose --ipc-show-output
    ./cli.py fuzz -s x86/isa_spec/base.json --nonstop --ruby --protean=Delay --protean-pred-mode=Predict --protean-pred-size=1024 -i 140 -n 200 -c $YAML -p protean-$i 2>&1 >$RVZR_DIR/src/logs/protean-$i.txt &
    echo "Launched protean-$i - Check $RVZR_DIR/src/logs/protean-$i.txt for results";
done
exit 0
...
TEST=PROTEAN-ST-SIMPLE-ARCHSEQ;
...
do
  echo "$TEST run $i launched"
  # nohup
  python3.11 ./cli.py fuzz -c $YAML -s x86/isa_spec/base.json -i 70 -n 200 --ruby --protean=None --protean-pred-mode=Predict --protean-pred-size=1024 -p ${TEST}_core$i &> $RVZR_DIR/${TEST}_core$i.out &
done
